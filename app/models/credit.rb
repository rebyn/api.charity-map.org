...
class Credit < ActiveRecord::Base
  ...
  def credit_to_belong_to_a_master_transaction
    errors.add(:master_transaction_id, 'has to belong to a Transaction') unless Transaction.exists?(uid: master_transaction_id)
  end
  ...
  def url
    "#{BASE_URI}/credits/#{uid}"
  end

  def merchant
    @transaction = Transaction.find_by_uid(master_transaction_id)
