  ...
  describe 'With Token' do
    ...
    describe 'GET index' do
      ...
      it 'returns User with transactions' do
        @params = { from: 'merchant@company.com', to: 'cuong@individual.net', amount: 100_000, currency: 'VND' }
        ...
        expect(response.body).to have_node(:amount).with(100_000.0)
    ...
    describe 'PUT/POST index invalid' do
      it 'should not create a new transaction with invalid params' do
        ...
        @params = { from: 'unregistered_merchant@company.com', to: 'cuong@individual.net', amount: 100_000, currency: 'VND' }
        post :index, @params
        expect(response.body).to eq(
          { error: 'Sender Email Not Found.' }.to_json)
        expect(response.status).to eq(400)
        # :from account doesn't have enough credit
        @params = { from: 'cuong@individual.net', to: 'love@social.org', amount: 100_000, currency: 'VND' }
        ...
        @params = { from: 'merchant@company.com', to: 'cuong@individual.net', amount: 100_000, currency: 'VND' }
        post :index, @params
        @params = { from: 'cuong@individual.net', to: 'merchant@company.com', amount: 100_000, currency: 'VND' }
        post :index, @params
        expect(response.body).to eq(
        ...
        @params = { from: 'love@social.org', to: 'love@social.org', amount: 100_000, currency: 'VND' }
        post :index, @params
        expect(response.body).to eq(
    ...
    describe 'PUT/POST index invalid' do
      ...
      it 'should create a new transaction with valid params' do
        ...
        @params = { from: 'cuong@individual.net', to: 'social@social.org', amount: 100_000, currency: 'VND' }
        post :index, @params
        expect(response.status).to eq(201)
        ...
        Credit.find_by(master_transaction_id: @transaction.uid, amount: 100_000).user.should eq(@org)
      end

      it 'should create a new transaction with valid params (+2 credits)' do
        @params = { from: 'merchant@company.com', to: 'cuong@individual.net', amount: 100_000, currency: 'VND' }
        post :index, @params
        @params = { from: 'merchant@company.com', to: 'cuong@individual.net', amount: 50_000, currency: 'VND' }
        post :index, @params
        @params = { from: 'cuong@individual.net', to: 'social@social.org', amount: 125_000, currency: 'VND' }
        post :index, @params
        get :authorize, token: Transaction.unauthorized.first.token.value
      ...
      it 'should create a new transaction with valid params (+3 credits)' do
        ...
        @params = { from: 'merchant@company.com', to: 'cuong@individual.net', amount: 50_000, currency: 'VND' }
        post :index, @params
        @params = { from: 'merchant@company.com', to: 'cuong@individual.net', amount: 25_000, currency: 'VND' }
        post :index, @params
        @params = { from: 'cuong@individual.net', to: 'social@social.org', amount: 170_000, currency: 'VND' }
        post :index, @params
        get :authorize, token: Transaction.unauthorized.first.token.value
      ...
      it "should create user if to[email] isn't on the system" do
        @params = { from: 'merchant@company.com', to: 'non_existent_user@individual.net', amount: 100_000, currency: 'VND' }
        post :index, @params
